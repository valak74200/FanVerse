...
interface SocialBettingMVPProps {
  userTokens: number
  onBetPlace: (betId: string, amount: number, prediction: string) => void
}

export function SocialBettingMVP({ userTokens, onBetPlace }: SocialBettingMVPProps) {
    ...
    <div className="h-full p-6 space-y-6">
          ...
          <Card className="bg-gradient-to-r from-yellow-500/20 to-orange-500/20 border-yellow-500/30 px-4 py-2">
            <div className="flex items-center space-x-2">
              <Coins className="w-5 h-5 text-yellow-400" />
                              <span className="font-bold text-yellow-400">{userTokens.toLocaleString('en-US')}</span>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="text-xs text-gray-400 mt-1">
                          Max: {userTokens.toLocaleString('en-US')} CHZ
                        </div>
                      </div>
      ...
      <Card className="bg-black/40 backdrop-blur-xl border-purple-500/30 p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-purple-400">
                              {activeBets.reduce((sum, bet) => sum + bet.pool, 0).toLocaleString('en-US')}
            </div>
