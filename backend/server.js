...
function generateNewBet() {
    ...
    io.emit('new_bet', { 
        ...
        totalPot: currentBet.totalPot || 0
    });
...
io.on('connection', (socket) => {
    ...
    socket.on('authenticate', async ({ userId }) => {
        try {
            ...
            // Envoyer le pari actuel s'il y en a un
            if (currentBet && currentBet.status === 'open') {
                console.log(`[PARI] Envoi du pari actuel à ${userId}: "${currentBet.question}"`);
                socket.emit('new_bet', { 
                    question: currentBet.question, 
                    options: currentBet.options,
                    duration: currentBet.duration,
                    timeRemaining: currentBet.duration - (Date.now() - currentBet.createdAt),
                    totalPot: currentBet.totalPot || 0
                });
            }
        } catch (error) {
    ...
    socket.on('place_bet', async ({ optionIndex, amount }) => {
        console.log(`[PARI] Tentative de pari de ${socket.userId}: option ${optionIndex}, montant ${amount}`);
        
        if (!socket.userId || !currentBet || currentBet.status !== 'open') {
            console.log(`[PARI] Pari refusé - Paris fermés pour ${socket.userId}`);
            return socket.emit('bet_error', { message: 'Les paris sont fermés.' });
        }
        if (currentBet.bets[socket.userId]) {
            console.log(`[PARI] Pari refusé - ${socket.userId} a déjà parié`);
            return socket.emit('bet_error', { message: 'Vous avez déjà parié.' });
        }
        if (amount > socket.chzBalance) {
            console.log(`[PARI] Pari refusé - Solde insuffisant pour ${socket.userId}: ${amount} > ${socket.chzBalance}`);
        ...
        try {
            ...
            if (user) {
                ...
                currentBet.totalPot = (currentBet.totalPot || 0) + amount;
                console.log(`[PARI] Pari accepté pour ${socket.userId}: ${amount} CHZ sur "${currentBet.options[optionIndex]}"`);
                
                // Envoyer la mise à jour du pot à tous les clients
                io.emit('bet_pot_update', {
                    totalPot: currentBet.totalPot,
                    betCount: Object.keys(currentBet.bets).length
                });
                ...
                console.log(`[PARI] Pari refusé - Transaction échouée pour ${socket.userId}`);
            ...
            socket.emit('bet_error', { 
                message: 'Erreur serveur lors du pari.',
...
server.listen(PORT, () => {
    ...
    setTimeout(() => {
        console.log('[INIT] Démarrage du système de paris automatique...');
        generateNewBet();
    }, 5000);
});
